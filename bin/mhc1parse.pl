#! usr/bin/perl
	...
	if($conda_env eq 'NONE'){
		...
		$exepath = $1;
		if($exepath eq ''){
			print "\nPath for OCTAVE is not set!";
			exit;
		}
		$exepath.='octave';
	}elsif($conda_env eq ''){
		print "\n Path/conda environment for OCTAVE is not properly set!\n";
		exit;
	}
	else{
	...
	unless (-e "err.txt"){print "\nEnvironment $conda_env does not exist!\n"; `rm err.txt`; exit;}
		...
		$pkgchk = `conda run -n $conda_env octave bin/fuzzpkg.m`;
		#print $pkgchk;
		if($pkgchk=~/fuzzy\-logic\-toolkit/){ $ok;} else{ print "\n Fuzzy-logic-toolkit is not installed\n"; exit;} 
	}
	
	
#PATH CHECK ABOVE FOR OCTAVE	
...
foreach $x(@pep){
	...
	$FuzzFile.= "$Fuzz_Blast,$Fuzz_TAP,$Fuzz_CLEV,$Fuzz_Tcell,$Fuzz_Binding\n";
...
#	}
	#Fuzzy End
	$finout.= $i."-".($i+8).",".$x.",".(($FS+($FW/2))/$FP).",$HLA_F,".(($NS+($NW/2))/$NP).",$HLA_N,".(($SS+($SW/2))/$SP).",$HLA_S,$uniHLA,$PC,".(((((($FS+($FW/2))/$FP)*100)+((($NS+($NW/2))/$NP)*100)+((($SS+($SW/2))/$SP)*100))/3)+$PC).",$B1,".((100-($B2/9)*100)).",".(($TPSCnt/13)*100).",".($Arr_NTCL[1]*100).",".($Arr_NTCL[2]*100)."\n";#.$Fuzzy_OUT.",".$CAT_SCORE.",".$Fuzz_Crisp."\n";
...
open(FILE,">temp/fuzzfile.csv");
print FILE $FuzzFile;
close(FILE);


$pr = `conda run -n OCTAVE octave bin/fuzzy.m`;
...
for ($x=0;$x<@spfuzz;$x++){
	...
	$CAT_SCORE = (($tscf[4]/100)*($tscf[3]+$tscf[1]+$tscf[0]+$tscf[2])*$Fuzzy_OUT);
	if($Fuzzy_OUT>=0.75){
		$Fuzz_Crisp = "Very High";
	}elsif($Fuzzy_OUT <0.25){
		$Fuzz_Crisp = "Low";
